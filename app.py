...
def create_sale():
    ...
    try:
        ...
        # Process items with their prices (including custom prices)
        processed_items = []
        for item in items:
            product = Product.query.get(item['id'])
            if product:
                # Use custom price if provided, otherwise use product's selling price
        ...
        # Create sale items with the custom prices
        for processed_item in processed_items:
            ...
            sale_item = SaleItem(
                ...
                unit_price=processed_item['price'],  # Use custom price
                total_price=processed_item['total']
            )
            db.session.add(sale_item)

            product.current_stock -= processed_item['quantity']
...
@app.route('/api/add_to_cart_custom', methods=['POST'])
@login_required(roles=['cashier', 'manager', 'admin'])
def add_to_cart_custom():
    ...
    price_to_use = custom_price if custom_price else product.selling_price
    ...
    if existing_item:
        ...
        # Update the price if custom
        existing_item['custom_price'] = price_to_use
    else:
        cart.append({
            'id': product.id,
            'name': product.name,
            'price': product.selling_price,
            'custom_price': price_to_use,
            'quantity': 1
        })
...
@app.route('/api/add_to_cart_simple_custom', methods=['POST'])
@login_required(roles=['cashier', 'manager', 'admin'])
def add_to_cart_simple_custom():
    data = request.get_json()
    barcode = data.get('barcode')
    custom_price = data.get('price')
